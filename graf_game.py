...
from res.colors import color_player, color_way, color_wall, color_start, color_finish, color_trace
from res.sizes import border, width, width_line, width_walls, width_window, height_window, height
from res.counters import record_time, score, t, k
from res.binary_flags import info, trace, flag_game
...
def create_graf_labirint(matrix):
    '''Создание графа лабиринта'''
    graf = {}

    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            graf[f'step{i+1}'] = matrix[i][j]
...
def search_FIFO(m, finish):
    ...
    while p != finish:
        ...
        p = m_fr.pop(0)
        if not p in m_checked:
            if len(m) > p[0] * 2 + 2:
                if m[p[0] * 2 + 1][p[1] * 2]:
                    if not (p[0]+1, p[1]) in m_fr and not (p[0]+1, p[1]) in m_checked:
                        m_fr.append((p[0]+1, p[1]))
            if -1 < p[0] * 2 - 2:
                if m[p[0] * 2 - 1][p[1] * 2]:
                    if not (p[0] - 1, p[1]) in m_fr and not (p[0] - 1, p[1]) in m_checked:
                        m_fr.append((p[0] - 1, p[1]))
            if len(m[0]) > p[1] * 2 + 2:
                if m[p[0] * 2][p[1] * 2 + 1]:
                    if not (p[0], p[1]+1) in m_fr and not (p[0], p[1]+1) in m_checked:
                        m_fr.append((p[0], p[1]+1))
            if -1 < p[1] * 2 - 2:
                if m[p[0] * 2][p[1] * 2 - 1]:
                    if not (p[0], p[1] - 1) in m_fr and not (p[0], p[1] - 1) in m_checked:
                        m_fr.append((p[0], p[1]-1))
            m_checked.append(p)
            pygame.draw.circle(window, color_trace, (border + p[0] * (width_line + width_walls) + width_line // 2,
                                                     border + p[1] * (width_line + width_walls) + width_line // 2),
                               width_line // 3 - 3)
            # draw_player()
